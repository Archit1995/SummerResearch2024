...
while(correct_degree) do
    ...
    points:=[seq(generate_random_vector(numelems(vars),p),i=1..num_points)];# sigma 
    print("points: ",points);
    shift_:=[seq(generate_random_vector(numelems(vars),p),i=1..num_points)];# beta\
    print("shift_: ",shift_);
    Y:=[seq(B(points[i],p),i=1..num_points)];
    
    m:=Expand(product(x-points[i][1],i=1..numelems(points))) mod p;
    print("m: ",m);
    points_for_uni_interp:=[seq(points[i][1],i=1..num_points)];
    print("points_for_uni_interp: ",points_for_uni_interp);
    u:=Interp(points_for_uni_interp,Y,x)mod p:
     print("u: ",u);
    checking:=[seq(Eval(u,x=points[i][1]) mod p,i=1..num_points)];
    print("Y: ",Y);
    ...
    if num_points = 128 then break; end if;
    if dq > 1 then 
        break:
    else 
