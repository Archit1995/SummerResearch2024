MRFI:=proc(B,num_vars::integer,vars::list,p::integer)
    ...
    while true do
        ...
        for j from T_old to 2*T-1 do     
            ...
            f,g,lg:=NDSA(B,sigma_[j],shift_,num_var,p,num_points_mqrfr): 
    ...
    coeff_num:=coeff_num*den_lc_inv mod p:
    coeff_den:=coeff_den*den_lc_inv mod p:
    ...
    return coeff_num,num_mono, coeff_den,den_mono:
end proc:
