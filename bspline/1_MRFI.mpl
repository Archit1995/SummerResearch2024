MRFI:=proc(B,num_vars::integer,num_eqn::integer,vars::list,p::integer)
    ...
    # lprint("numerator_done: ",numerator_done):
    ...
    # T_old:=0:
    T_old:=1:
    # lprint("Yuhuuu", NDSA(B,[seq(1,i=1..num_vars)],shift_,num_vars,p,T)):
    ...
    lprint("lin_sys: ",lin_sys):
    lprint("nops mqrfr_results: ",nops(mqrfr_results)):
    ...
    if lin_sys = false then 
        ...
        lprint("deg_num: ",deg_num):
        lprint("deg_den: ",deg_den):
   ...
   count:=0:
    while true do
        ...
        for j from T_old to 2*T-1 do   #powers of prime [2^j,3^j,...,p^j]
            ...
            lprint("sigma_[",j,"]=",sigma_[j]):
            lprint("F: ",F):
            lprint("nops(F)=",nops(F)):
    ...
    #     # Call BMEA only on the polynomial which caused the done, while keeping the value of the other polynomial.
        ...
        if not(denominator_done) then
            ...
            lprint("denominator_done: ",denominator_done):
        end if;
    ...
    #     lprint("num of roots of lambda_num =",nops(R_num)):
    #     lprint("num of roots of lambda_den =",nops(R_den)):
    #     if nops(R_num)=terms_num and terms_num < T then 
    #         lprint("Numerator Success!!"):
    #         numerator_done:=true:
    #     end if:
    #     if nops(R_den)=terms_den and terms_den < T  then 
    #         lprint("Denominator Success!!"):
    #         denominator_done:=true:
    #     end if:
    #     if numerator_done and denominator_done then 
    #         lprint("IN TERMINATION oF GET_NUM_TERMS_LAMBDA_ROOTS");  
    ...
    # lprint("den_lc= ",den_lc):
    # lprint("den_lc_inv= ",den_lc_inv):
