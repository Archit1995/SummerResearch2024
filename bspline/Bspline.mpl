...
pars := convert(params,list);
with(LinearAlgebra):
B := ReducedRowEchelonForm(A):
# B is RREF of [A|b]. Why RREF? Det of RREF matrix is product of diagonal elements
print("B = ", B);
print("B[1] = ", B[1]);
# n:=3;
n := numelems(Vars);
...
# From here out we are using Bareiss Edmonds fraction free Gaussian elimination
# C := A[1..21,1..21]:# coeff matrix of the system, A is augmented matrix
C:=A[1..n,1..n];
print("C = ", C);
print("C[1] = ", C[1]);
...
print("b = ", b):
# print("b[1] = ", b[1]):
# Augmented Coeff where the last column of C is replaced with b 
# print("<C[1..21,1..20]|b> = ", <C[1..21,1..20]|b>):

print("[C[1..n,1..n-1|b]",<C[1..n,1..n-1]|b>);
print("gcd(D,N) = ",gcd(D,N)):

N := Determinant(<C[1..n,1..n-1]|b>):
# N := Determinant( <C[1..21,1..20]|b> );
N_:=expand(N):
print("N = ", N_);
print("nops(N_) = ", nops(N_)):
factor(N/D);
