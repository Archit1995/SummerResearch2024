...
MRFI_Vector:=proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    ...
    while true do
        ...
        if common_den_flag then
            ...
            if not denominator_done[1] then
                ...
                lambda_den_eval[1] := [ temp]:
                terms_den_eval[1] :=degree(temp, Z):
                R_den_eval[1] := Roots(temp) mod p:
                lprint("MRFI lambda_den_eval: ",lambda_den_eval[1]):
                # terms_den_eval := [degree(temp, Z)]:
                lprint("MRFI terms_den_eval: ",terms_den_eval[1]):
                # R_den_eval := [Roots(temp) mod p]:
                lprint("MRFI R_den_eval: ",R_den_eval[1]):
    ...
    # print("lambda_den_eval: ",lambda_den_eval):
    for k from 1 to num_eqn do
        num_mono[k] := generate_monomials(Roots_num_eval[k], num_vars, Primes, vars):
        # temp := generate_monomials(Roots_num_eval[k], num_vars, Primes, vars):
        if num_mono[k] = FAIL then return FAIL: end if:
        coeff_num[k] := Zippel_Vandermonde_solver(num_eval[k], terms_num_eval[k], 
                                     Roots_num_eval[k], lambda_num_eval[k][1], p):
    ...
    if common_den_flag then
        den_mono[1] := generate_monomials(Roots_den_eval[1], num_vars, Primes, vars):
        if den_mono[1] = FAIL then return FAIL: end if:
        
        coeff_den[1] := Zippel_Vandermonde_solver(den_eval[1], terms_den_eval[1],
                                               Roots_den_eval[1], lambda_den_eval[1][1], p):
        # final_den[1] := construct_final_polynomial(coeff_den[1], den_mono[1]):
        
    else
        for k from 1 to num_eqn do
            den_mono[k] := generate_monomials(Roots_den_eval[k], num_vars, Primes, vars):
            if den_mono[k] = FAIL then return FAIL: end if:
            # den_mono_list := [op(den_mono_list), temp]:
            coeff_den[k] := Zippel_Vandermonde_solver(den_eval[k], terms_den_eval[k],
                                                   Roots_den_eval[k], lambda_den_eval[k][1], p):
