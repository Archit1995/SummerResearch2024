...
get_data:=proc(test_case)
    ...
    elif test_case = "vector_example" then 
        Sys:=[x1=(y1^2-2*y1+3)/(y1*y2-1), x2= randpoly([y1, y2], degree = 11) /randpoly([y1, y2], terms = 7) mod p];
        vars:=[x1,x2]:
        return Sys, vars, [y1,y2]:
    ...
    return Sys,Vars,convert(params,list):
end proc:
