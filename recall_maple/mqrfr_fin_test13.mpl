...
# r_:=rand(100..2^67-1):
# points__:=[seq(r_(),i=1..num_points)]:
# points:=map(modp,points__,p):
...
# [seq(Eval(h,x=i) mod p,i=points[1..numelems(points)])]:
#  same ys as original polynomial so we need to run 500 iterations to get the correct polynomial
