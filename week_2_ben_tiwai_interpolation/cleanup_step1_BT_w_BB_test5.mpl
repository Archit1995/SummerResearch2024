...
B:=proc(a,var,point_)
    # local c;
    # c:= a/b;
    var_eval:=table([var[v]=point_[v] for v in [1..numelems(point_)]]);
    for v from 1 to numelems(point_)do 

    return seq(eval(a,x=i),i=point_[1..numelems(point_)]);
end proc;
...
eval_prime_powers:=proc(B,T,prime_points)
    # y:=Vector(2*T-1,0);
    local y;
    for i from 0 to 2*T-1 do
        y[i+1]:=[seq(prime_points[j]^i,j=1..num_var)];
    end do;
...
# v:=eval_prime_powers(B,T,prime_points);
whattype(v):
